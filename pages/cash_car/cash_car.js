      ...
      if (that.data.travel != "") {
        ...
        if (reg.test(that.data.people)) {
          ...
          if (reg_0.test(that.data.number)) {
            http({ s: "Car.recover", plate: that.data.pingpai, mileage: that.data.travel, date: that.data.year + '-' + that.data.mouth + '-' + that.data.day, owner: that.data.people, phone: that.data.number}).then(res => {
              if (res.data.ret == 200) {
                $Toast({
                  content: '提交成功',
                  type: 'success'
