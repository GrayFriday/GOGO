...
Page({
  ...
  sumbit() {
    ...
    if (that.data.pingpai == "" || that.data.cp == "" || that.data.sele_color.colorname == "" || that.data.seleXh == "") {
      ...
      http({ s: "Car.doupd", id: wx.getStorageSync('sele_car_id'), carnum: that.data.cp, plate: that.data.pingpai, plateid: plateid, colorid: that.data.color_id, cartype: that.data.car_type}).then(res => {
        console.log(res)
        if (res.data.ret == 200) {
          $Message({
            content: '修改成功',
